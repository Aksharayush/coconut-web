...
import { MessageSquare, Users, UserPlus, Settings, Menu, ChevronLeft, Send, Check, LogOut, Globe, X, CornerDownRight } from 'lucide-react'
import {
...
import { getFirestore, collection, query, orderBy, onSnapshot, addDoc, serverTimestamp, where, doc, setDoc, updateDoc } from "firebase/firestore"
import { getDatabase, ref, onValue, push, set, serverTimestamp as rtdbTimestamp } from "firebase/database"
...
export default function CoconutApp() {
  ...
  // Unified Message Listener (RTDB for Common, Firestore for Private)
  useEffect(() => {
    setMessages([]);
    if (chatType === 'common') {
  ...
  return (
    <div className="bg-zinc-950 min-h-screen text-white flex font-sans overflow-hidden">
      ...
      {/* SIDEBAR */}
      <aside className={`fixed inset-y-0 left-0 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 w-72 bg-zinc-900 border-r border-zinc-800 z-30 flex flex-col`}>
        <div className="p-6 font-black italic text-2xl flex items-center gap-2"><div className="w-3 h-3 bg-emerald-500 rounded-full animate-pulse" /> COCONUT</div>
        <nav className="flex-1 px-4 space-y-1">
          <button onClick={() => setView('home')} className={`w-full flex items-center gap-3 p-3 rounded-lg ${view === 'home' ? 'bg-zinc-800' : 'text-zinc-500'}`}><MessageSquare size={18}/> Dashboard</button>
          <button onClick={() => setView('friends')} className={`w-full flex items-center gap-3 p-3 rounded-lg ${view === 'friends' ? 'bg-zinc-800' : 'text-zinc-500'}`}><Users size={18}/> Friends</button>
          <button onClick={() => setView('settings')} className={`w-full flex items-center gap-3 p-3 rounded-lg ${view === 'settings' ? 'bg-zinc-800' : 'text-zinc-500'}`}><Settings size={18}/> Settings</button>
        </nav>
      ...
      <div className="flex-1 flex flex-col h-screen">
        {view === 'home' && (
          <main className="p-6 md:p-10 max-w-5xl mx-auto w-full space-y-10 overflow-y-auto">
            <header><p className="text-zinc-500 font-mono text-xs uppercase tracking-widest">Operator</p><h2 className="text-4xl font-bold tracking-tighter">{username}</h2></header>
            ...
            <section>
              ...
              <div onClick={() => { setChatType('common'); setView('chat'); }} className="group bg-zinc-900 border border-zinc-800 p-8 rounded-2xl cursor-pointer hover:border-emerald-500 transition-all relative overflow-hidden">
                <div className="relative z-10 flex justify-between items-center">
                  <div><h4 className="text-2xl font-black italic underline decoration-emerald-500"># Coconut-chat</h4><p className="text-zinc-500 text-sm mt-1 font-medium">Stay Hydrated ! </p></div>
            ...
            <section className="bg-zinc-900 border border-zinc-800 p-8 rounded-2xl max-w-md">
                <h4 className="font-bold mb-4 flex itms-center gap-2 uppercase text-xs tracking-widest text-zinc-400"><UserPlus size={16}/> Connect with user</h4>
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {messages.map((msg) => (
                <div key={msg.id} onDoubleClick={() => setReplyingTo(msg)} className={`flex flex-col group cursor-pointer ${msg.sender === username ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`max-w-[85%] p-3 px-5 rounded-2xl text-sm transition-all active:scale-95 ${msg.sender === username ? 'bg-emerald-600 rounded-tr-none' : 'bg-zinc-900 border border-zinc-800 rounded-tl-none'}`}>
                    {msg.replyTo && (
            ...
            <div className="p-4 bg-zinc-900 border-t border-zinc-800">
              ...
              <form onSubmit={sendMessage} className="max-w-4xl mx-auto flex gap-2">
                ...
                <button type="submit" className="bg-emerald-600 p-3 px-8 rounded-full font-black text-xs hover:bg-emerald-500 flex items-center gap-2"><Send size={16}/> SEND</button>
              </form>
            </div>
          </div>
        )}
        ...
        {view === 'friends' && (
            <div className="p-10 h-full max-w-2xl mx-auto w-full">
                ...
                {pendingInvites.map(inv => (
                    <div key={inv.id} className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => acceptInvite(inv)} className="bg-emerald-600 px-6 py-2 rounded-xl text-xs font-black uppercase hover:bg-emerald-500">Establish Link</button>
                    </div>
                ))}
                {pendingInvites.length === 0 && <div className="text-center py-20 border-2 border-dashed border-zinc-900 rounded-3xl text-zinc-600 font-bold italic">NO PENDING FREQUENCIES</div>}
            </div>
        )}
